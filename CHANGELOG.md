...
the thinnest tiers in the live packs.

| Pack | Added | Tier it fills | Now |
|---|---|---|---|
| Theory Fundamentals | 8 medium | medium 18 → 26 | 67 rows, 28 / 26 / 13 |
| Advanced Theory | 5 easy | easy 10 → 15 | 44 rows, 15 / 20 / 9 |
| Bass | 8 hard | hard 15 → 23 | 106 rows, 30 / 53 / 23 |
| Seventies | 4 hard, 5 easy Italy-only | English hard 17 → 21, Italy-only easy 1 → 6 | 145 rows |

Triage edits: the ii–V–I blurb now uses Autumn Leaves; the Duck Dunn stem
drops the pipe; the Geezer Butler row now asks which song started as
Walpurgis (the old stem gave the answer away); the Kind of Blue blurb is about
the "so what" horn answer. Eighties: the Hands Across America row asks for the
event instead of giving it in the stem.

## v0.210.120: triage rescan installed for seven more packs

Every change in the triage tool is installed as approved for Eighties, Guitar
Gods, Bass, Theory, Advanced Theory, Beatles and Guitar Technique. Italian is
written for every question, option and blurb whose English changed. Each pack
passed the pack gate and the round-trip install check.

| Pack | Rows (was) | Easy / Med / Hard |
|---|---|---|
| Eighties | 145 (134) | 63 / 55 / 27 |
| Guitar Gods | 80 (90) | 27 / 35 / 18 |
| Bass | 98 (104) | 30 / 53 / 15 |
| Theory | 59 (60) | 28 / 18 / 13 |
| Advanced Theory | 39 (40) | 10 / 20 / 9 |
| Beatles | 88 (90) | 33 / 39 / 16 |
| Guitar Technique | 101 (104) | 38 / 40 / 23 |

27 rows cut, 74 question stems rewritten and 15 hard Eighties rows added. The old versions of all 101 are
archived verbatim in `quiz_removed_v0.210.120.json` in the project.

Calls made without asking (reverse any of these from the archive):
- Cross-pack doubles kept on both sides: gallop, chorus, Merle Travis, Django,
  Leo Fender.
- Chucking: Guitar Gods #31 cut, Technique keeps it.
- Albert King: Technique #52 cut, Guitar Gods keeps it.
- Charlie Christian: Technique #86 cut, Guitar Gods keeps it.
- Technique #20 (where to fret): options rewritten around the fret wire.
- Old Eighties cloud proposals #43, #104, #106, #123, #126, #129, #130 and
  #131 were not applied; the tool's own rows stand.

Gate fixes during install:
- Eighties: the Thriller video row now lists titles only, so it no longer
  shares an answer string with the Thriller album row.
- Appetite for Destruction and Born in the U.S.A. rows got longer distractors
  (Terence Trent D'Arby's debut; Huey Lewis and the News) so the answer is not
  the one long option.
- Eighties Italian: the WrestleMania and 1984 Olympics rows had their Italian
  swapped with each other since an earlier build. Each row now has its own.

Quiz help text no longer promises "over a thousand questions across nineteen
packs". Players only get the 8 ready packs (731 questions), so it now says
"hundreds of questions, and more packs on the way" in both languages.

Eighties hard tier filled: 15 new hard rows in both languages, each fact
checked and approved in triage. Shared: Becker at Wimbledon, the Velvet
Revolution, Manic Monday (Prince as Christopher), Amadeus, Cinema Paradiso,
Graceland, Sally Ride, Blue Velvet, Geldof in The Wall, Time's 1982 Machine of
the Year, Blue Monday, Prost's first title. Italy only: Si può dare di più,
Raf's original Self Control, Tomba at Calgary. Eighties is now 145 rows,
63/55/27; a hard round draws on 23 hard questions in English and 24 in
Italian, and Italy-only hard goes from 1 to 4.

Two English-only hard rows replaced: the Hurricane Gilbert "first live
hurricane" row (live hurricane TV goes back to 1961) and the Cadillac Cimarron
"worst of the decade" row (no such title; blurb quotes unsourced). In their
place: Bill Buckner's 1986 World Series error and Hands Across America. US-only
hard stays at 3.

## v0.210.119: iPhone mic crash fix, Seventies triage installed, theory generator pass

iPhone: opening Tuner, Tools or Metronome from the chooser reset the screen
with "Something went wrong" (log: `"IntonareMic" plugin is not implemented on
ios`, every iOS build since v0.185). The native mic plugin only exists on
Android. On iOS the native start failed and fell back to the WebView mic as
designed, but the cleanup's `p.stop()` rejection was never caught and hit the
global error net, which resets the screen. Native mic is now gated on
`Capacitor.isPluginAvailable('IntonareMic')`, so iOS goes straight to the
WebView path (and picks up a Swift plugin automatically once one exists), and
`_nativeMicStop` catches every plugin promise. Simulated iOS: no dialog on any
mic module. Simulated Android: native path still starts. Two sentinel pins added.

Seventies replaced with the triaged pack: 136 questions (was 113), 50/62/24,
both languages. Fact-bait rows rewritten to ask for the famous thing, blurbs
rewritten to one idea each and fact-checked, 7 cut, 8 new rows. The 22
replaced shipped rows are archived verbatim in `quiz_removed.json`.

Theory question generator: English says half step and whole step, not
semitone and tone (scale names unchanged). Interval, inversion, harmonic
major, 13th and vi-IV-I-V blurbs rewritten; four fact-pool lines replaced;
cadence labels Authentic / Half / Deceptive. Advanced Theory no longer deals
grade-one cards (`mqAdvLevel`). Meter blurbs agree in number ("holds" /
"hold").
